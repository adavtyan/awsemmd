...
def create_project(parametersobject):
	...
	for orientation in range(1,number_of_orientations+1):
		...
		results = create_random_pdb(separation_distance = separation_distance, move_chain_id = move_chain_id, fix_chain_id = fix_chain_id, input_file_name = Initial_dimer_pdb, output_pdb_name = output_pdb_name, model_number = model_number)
	...
	for orientation in range(1,number_of_orientations+1):
		...
		os.system("python2 "+location+' '+file_name_start+" "+file_name_start+".coord")
		location = os.path.normpath(Path_to_awsem+"create_project_tools/CoordinatesToWorkLammpsDataFile.py")
		os.system("python2 "+location+" "+file_name_start+".coord "+file_name_start+".data -b")
	...
	os.system('python2 '+location+' > ssweight')
