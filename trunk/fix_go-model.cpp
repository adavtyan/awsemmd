...
void FixGoModel::compute_dihedral(int i)
{
	...
	phi = -atan2(Y, X); //add "-" to be consisitent with the input generated by GetContactMapFromPDB.py
						//which generate additional negative sign for the native dihedrals
...
void FixGoModel::compute_contact_gaussian(int i, int j)
{
	...
	rsq = dx[0]*dx[0]+dx[1]*dx[1]+dx[2]*dx[2];
	r = sqrt(rsq);
	w_sq_inv = 1.0/pow( abs(i_resno - j_resno), 0.3 ) / gaussian_width / gaussian_width ;
