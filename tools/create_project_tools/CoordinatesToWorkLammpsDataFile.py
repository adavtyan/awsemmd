...
class Atom:
    ...
    def write_(self, f):
        ...
        f.write("\t".join([str(self.ch), str(self.res), str(self.ty), str(
            self.q), str(self.x), str(self.y), str(self.z)]))
        f.write("\n")
...
for l in inp:
        ...
        if desc == 'N0' or desc == 'N':
            ...
            if cg and cg_bonds:
                if last_nos['C-Alpha'] != 0 and last_chno['C-Alpha'] == chain_no:
                    n_bonds += 1
                    bonds.append(
                        Bond(n_bonds, 1, last_nos['C-Alpha'], n_atoms))
            ...
            if cg and cg_bonds:
                if last_nos['C-Alpha'] != 0 and last_chno['C-Alpha'] == chain_no:
                    n_bonds += 1
                    bonds.append(
                        Bond(n_bonds, 2, last_nos['C-Alpha'], n_atoms))
            atom_type = 3
            last_nos[desc] = n_atoms
            last_chno[desc] = chain_no
            group_id = 3
    ...
    if not go:
        if desc == 'C-Beta' or desc == 'H-Beta' or desc == 'C-Alpha' or desc == 'O':
            #            n_atoms += 1
            atoms.append(Atom(n_atoms, chain_no, n_res, atom_type,
                              0.0, float(l[3]), float(l[4]), float(l[5])))
...
inp = open(myhome + "/programs/awsemmd/tools/create_project_tools/inFilePattern.data")
inFile = inp.read()
inp.close()
