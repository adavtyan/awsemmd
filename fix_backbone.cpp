...
//	if (burial_flag && Step>=sStep && Step<=eStep) fprintf(fout, "}\n\n");
	...
	if (Step>=sStep && Step<=eStep && amh_go_flag && forceCh) copyforce(tmp_force);
	tmp=foriginal[0];
	if (amh_go_flag)
			compute_amh_go_model();

	if (amh_go_flag && Step>=sStep && Step<=eStep) {
		fprintf(fout, "AMH_Go %d:\n", nn);
		fprintf(fout, "AMH_Go_Energy: %.12f\n", foriginal[0]-tmp);
		if (forceCh) {
			fprintf(fout, "AMH_Go_Max_Force_CA: %.12f\n", findBiggestForce(tmp_force, 1));
			fprintf(fout, "AMH_Go_Max_Force_CB: %.12f\n", findBiggestForce(tmp_force, 2));
			fprintf(fout, "AMH_Go_Max_Force_O: %.12f\n", findBiggestForce(tmp_force, 3));
		}
		out_xyz_and_force();
	}
